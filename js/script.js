...
document.querySelectorAll('nav.primary a[data-nav]').forEach(link => {
    ...
    // ------------------------- //
// Fixed Contact Button Scroll Logic
// ------------------------- //
...
function toggleContactBtn() {
    // If any element is missing from the HTML, stop to prevent errors
    if (!contactBtn || !aboutSection || !contactSection) return;

    const aboutTop = aboutSection.getBoundingClientRect().top;
    const contactTop = contactSection.getBoundingClientRect().top;
    const windowHeight = window.innerHeight;

    // Button shows up after scrolling past 'About' and hides once reaching 'Contact'
    if (aboutTop < windowHeight * 0.8 && contactTop > windowHeight * 0.5) {
